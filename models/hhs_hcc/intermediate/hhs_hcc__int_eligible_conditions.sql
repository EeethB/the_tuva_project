{{ config(
     enabled = var('hhs_hcc_enabled',var('claims_enabled',var('tuva_marts_enabled',False)))
   )
}}
/*
...
Claims filtering logic:
 ...
 - Outpatient:
    ...
    - CPT/HCPCS in CPT/HCPCS seed file from CMS
- Need to check if these reqs are the same for HHS - They're at least similar
- I also think these reqs should be their own seed possibly
...
Jinja is used to set payment and collection year variables.
 ...
 - The collection year is one year prior to the payment year.
	- Need to check what this means, and whether HHS model uses the same year logic 
*/

{% set payment_year_compiled = var('hhs_hcc_payment_year') -%}
...
with medical_claims as (
    ...
    select
        ...
        , bill_type_code
        , hcpcs_code
    from {{ ref('hhs_hcc__stg_core__medical_claim') }}
...
, conditions as (
    ...
    select
        ...
        , patient_id
        , code
    from {{ ref('hhs_hcc__stg_core__condition') }}
